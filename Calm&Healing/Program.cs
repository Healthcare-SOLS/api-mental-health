...
// Register factory to resolve dynamic schema context
builder.Services.AddScoped<Func<string, DynamicTenantDbContext>>(serviceProvider => schema =>
{
    var optionsBuilder = new DbContextOptionsBuilder<DynamicTenantDbContext>();
    var configuration = serviceProvider.GetRequiredService<IConfiguration>();
    var connectionString = configuration.GetConnectionString("DefaultConnection");

    optionsBuilder.UseNpgsql(connectionString);

    return new DynamicTenantDbContext(optionsBuilder.Options, schema);
});
